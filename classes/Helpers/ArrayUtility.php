...
class ArrayUtility
{
    ...
    public static function getValueByPath(array $array, $path, $delimiter = '/')
    {
        ...
        if (is_string($path)) {
            ...
            // Programming error has to be sanitized before calling the method -> global exception
            throw new \Exception('getValueByPath() expects $path to be string or array, "' . gettype($path) . '" given.', 1476557628);
        }
        ...
        foreach ($path as $segment) {
            if (is_array($value) && array_key_exists($segment, $value)) {
                ...
                throw new \Exception('Segment ' . $segment . ' of path ' . implode($delimiter, $path) . ' does not exist in array', 1341397869);
            }
        }
        return $value;
    }
    ...
    public static function removeByPath(array $array, $path, $delimiter = '/')
    {
        ...
        foreach ($path as $segment) {
            ...
            if (!array_key_exists($segment, $pointer)) {
                throw new \Exception('Segment ' . $segment . ' of path ' . implode($delimiter, $path) . ' does not exist in array', 1371758436);
            }
    ...
    public static function array_keys_exist($needles, array $haystack)
    {
        ...
        return self::values_in_array($needles, array_keys($haystack));
    }
